...
--- Creates a new plugin and returns a table to be populated with functionality.  Provides for automatic
--- embedding of libraries if NewModule is available.
...
local function enabledIterator(plugins, name)
    local pluginName, plugin = next(plugins, name)
    if pluginName and not plugin:IsEnabled() then
        -- skip disabled modules
        return enabledIterator(plugins, pluginName)
    end
...
function OnePlugin:SetBasePluginPrototype(prototype)
    if type(prototype) ~= 'table' then
        error(
            format("%s: 'prototype' - expected type 'table' got '%s.", usages.SetBasePluginPrototype, type(prototype)),
            2
        )
    end
...
for target, _ in pairs(OnePlugin.embedded) do
